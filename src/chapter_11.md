...
**数学陈述**：在最高一层，一个数学陈述只是一段文字，我们可以把它当成一个**二进制串** $x \in \{0, 1\}^*$。数学陈述包括一些断言，其正确性不依赖于任何经验试试，而只表示抽象概念的性质。
例如，下面是一个数学陈述：^[这是一个**错的**陈述]
> 数字 $2$,$696$,$635$,$869$,$504$,$783$,$333$,$238$,$805$,$675$,$613$, $588$,$278$,$597$,$832$,$162$,$617$,$892$,$474$,$670$,$798$,$113$都是质数。

数学陈述不一定要含有数字。他们可以断言其他任何数学对象的性质，包括几何，串，函数，图，甚至**程序**。
因此，数学陈述的另一个性质如下，
...
```
**证明系统**：对一个定理$x \in \{0, 1\}^*$的**证明**也是一个串$w \in \{0, 1\}^*$

1. 有效性 给定一个陈述$x$和一个证明$w$，存在一个算法来验证$w$是否是$x$的证明（比如一行一行的确认每一行都是由合法的推理规则从前一行推导出来的。

2. 可靠性 若对于$x$，存在一个合法的证明$w$，则$x$为真。

这些是对于一个证明系统的很少的要求。第二个要求（可靠性）正是形式系统的定义：你不应该能证明一个假的命题。第一个要求也是必要的。如果没有一组推理规则（换句话说，一个算法）来检查一个证明是否合法，那么1
